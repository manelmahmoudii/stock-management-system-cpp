...
export default function NotificationDropdown() {
  ...
  const { lowStockAlerts, outOfStockAlerts, hasUnreadAlerts, markAsRead, getAlertMessage, totalAlerts } = useStockAlerts();
  const [notifying, setNotifying] = useState(true);

  useEffect(() => {
    setNotifying(hasUnreadAlerts);
  }, [hasUnreadAlerts]);
            ...
            <>
              ...
              {outOfStockAlerts.map((product) => (
                <li key={`out-${product.id}`}>
                  ...
                  >
                    <span className="relative block w-full h-10 rounded-full z-1 max-w-10 bg-red-100 flex items-center justify-center">
              ...
              {lowStockAlerts.map((product) => (
                <li key={`low-${product.id}`}>
                  <DropdownItem
                    ...
                    className="flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5"
                  >
                    <span className="relative block w-full h-10 rounded-full z-1 max-w-10 bg-yellow-100 flex items-center justify-center">
